...
/** DFS on an UNDIRECTED graph with letter node IDs. */
public class UndirectedDFSLetters {
    public static void main(String[] args) {
        try {
            ...
            allLabels.add(srcLabel); allLabels.add(dstLabel);

            for (int i=0;i<M;i++) {
                String u=in.next(); String v=in.next();
                if (u==null||v==null) { System.out.println("No path"); return; }
                edgeLabels.add(new String[]{u,v});
                allLabels.add(u); allLabels.add(v);
            }

            if (allLabels.size()!=N) N=allLabels.size();
            Map<String,Integer> id=new HashMap<>();
            String[] labelOf=new String[N+1];
            int idx=1; for (String s: allLabels){ id.put(s,idx); labelOf[idx]=s; idx++; }
            ...
            for (String[] e: edgeLabels){
                Integer u=id.get(e[0]); Integer v=id.get(e[1]);
                if (u==null||v==null) continue;
                neigh[u].add(v);
                neigh[v].add(u); // undirected
            }

            int[][] adj=new int[N+1][];
            for (int i=1;i<=N;i++) adj[i]=neigh[i].stream().mapToInt(Integer::intValue).toArray();
            ...
            for(int i=0;i<path.size();i++){ if(i>0) sb.append(' '); sb.append(labelOf[path.get(i)]); }
